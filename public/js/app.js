...
function setStep(n) {
  ...
  // Update indicators (only steps 1-4)
  for (let i = 1; i <= 4; i++) {
    const item = document.getElementById(`step-ind-${i}`);
    if (!item) continue;
    item.classList.remove('active', 'done');
    if (i < n) item.classList.add('done'), item.querySelector('.step-circle').innerHTML = '✓';
    else if (i === n) item.classList.add('active');
    else item.querySelector('.step-circle').textContent = i;
  }
  state.currentStep = n;
...
function validateStep(s) {
  if (s === 1) {
    ...
    if (!state.barberId) {
      toast('error', 'Barbero requerido', 'Por favor elige un barbero.');
      return false;
    }
  }
  if (s === 2) {
    if (!state.date) {
      toast('error', 'Fecha requerida', 'Por favor selecciona una fecha.');
      return false;
    }
    if (!state.time) {
      toast('error', 'Horario requerido', 'Por favor selecciona un horario.');
      return false;
    }
  }
  if (s === 3) {
...
async function loadSlots() {
  ...
  try {
    ...
    if (!data.worksToday) {
      container.innerHTML = `<div class="slots-empty">🚫 El barbero no trabaja este día.<br><small style="color:var(--text-dim)">Barbero 1: Lun–Sáb | Barbero 2: Todos los días</small></div>`;
      return;
    }
  ...
  try {
    ...
    document.getElementById('conf-details').innerHTML = `
      <div class="booking-summary">
        ...
        <div class="summary-row"><span class="summary-label">Barbero</span><span class="summary-value">${data.appointment.barberId === 1 ? 'Barbero 1' : 'Barbero 2'}</span></div>
...
function renderApptDetails(appt) {
  const barberName = appt.barberId === 1 ? 'Barbero 1 (Lun–Sáb)' : 'Barbero 2 (Todos los días)';
  const isPast = new Date(`${appt.date}T${appt.time}:00`) < new Date();
  const isCancelled = appt.status === 'cancelled';

  // Calculate if can cancel (>24h)
  ...
  document.getElementById('appt-info').innerHTML = `
    <div class="appt-row"><label>Código</label><span style="font-family:monospace;letter-spacing:2px;color:var(--gold)">${appt.code}</span></div>
    ...
    <div class="appt-row"><label>Hora</label><span style="color:var(--gold);font-weight:600">${appt.time}</span></div>
    <div class="appt-row"><label>Estado</label>
      <span class="appt-status ${appt.status}">
        ${appt.status === 'confirmed' ? '✅ Confirmada' : '❌ Cancelada'}
      </span>
    </div>
    ${!canCancel && canModify ? `<div style="margin-top:.75rem;padding:.75rem;background:rgba(255,152,0,.08);border:1px solid rgba(255,152,0,.2);border-radius:8px;font-size:.8rem;color:#FF9800">⚠️ Solo puedes cancelar con 24h de anticipación.</div>` : ''}
  ...
  let actionsHtml = '';
  if (canModify) {
    actionsHtml += `<button class="btn btn-warning" onclick="openEditModal()" style="flex:1;justify-content:center">✏️ Modificar</button>`;
  }
  if (canCancel) {
    actionsHtml += `<button class="btn btn-danger" onclick="cancelAppointment()" style="flex:1;justify-content:center">🗑️ Cancelar</button>`;
  }
...
async function cancelAppointment() {
  ...
  try {
    ...
    if (!res.ok) {
      toast('error', 'No se pudo cancelar', data.error);
      return;
    }
...
async function loadEditSlots() {
  ...
  container.innerHTML = '<span class="spinner spinner-gold"></span>';
  ...
  try {
    ...
    // Include currently booked slot so user can keep it
...
async function submitEdit() {
  ...
  if (!date || !time) {
    toast('error', 'Datos incompletos', 'Selecciona fecha y horario.');
    return;
  }
